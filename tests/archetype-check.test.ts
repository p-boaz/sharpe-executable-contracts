...
import { archetypesFor } from "../src/pipeline/archetypes.js";
...
function loadFixture(dir: string) {
  const ir = JSON.parse(readFileSync(join(dir, "ir.json"), "utf8")) as ContractIR;
  return { ir, dir };
}

function loadScenario(dir: string, archetypeId: string): Scenario {
  return JSON.parse(
    readFileSync(join(dir, "scenarios", `${archetypeId}.json`), "utf8"),
  ) as Scenario;
}

test("credit-card deterministic fallbacks pass AND-form validation", () => {
  const { ir, dir } = loadFixture(CC_FIXTURE);
  const archetypes = archetypesFor("credit_card");
  for (const archetype of archetypes) {
    const scenario = loadScenario(dir, archetype.id);
...
test("lease deterministic fallbacks pass AND-form validation", () => {
  const { ir, dir } = loadFixture(LEASE_FIXTURE);
